...
const Navbar = () => {
  ...
  const linkStyle = (path) => ({
    ...
    textDecoration: 'none',
    color: isActive(path) ? goldLight : '#bbb',
  ...
  const links = role === 'admin' ? adminLinks : role === 'organizer' ? organizerLinks : role === 'attender' ? attenderLinks : [];
  ...
  return (
    <nav style={{
      ...
      <div style={{ maxWidth: 1400, margin: '0 auto', padding: '0.7rem 1.5rem', display: 'flex', alignItems: 'center', gap: 24 }}>
        {/* Logo */}
        <Link to="/" style={{ textDecoration: 'none', display: 'flex', alignItems: 'center', gap: 10, flexShrink: 0 }}>
          <div style={{
            ...
            <svg width="20" height="20" viewBox="0 0 24 24" fill="#0f0f0f" stroke="#0f0f0f" strokeWidth="0">
              <path d="M2 9a3 3 0 0 1 0 6v2a2 2 0 0 0 2 2h16a2 2 0 0 0 2-2v-2a3 3 0 0 1 0-6V7a2 2 0 0 0-2-2H4a2 2 0 0 0-2 2z"/>
            </svg>
          </div>
          <div>
            <div style={{ color: goldLight, fontWeight: 800, fontSize: 16, lineHeight: 1.1, letterSpacing: '-0.3px' }}>EventPro</div>
            <div style={{ color: '#555', fontSize: 10, letterSpacing: '0.5px' }}>TICKET PLATFORM</div>
          </div>
        </Link>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 4, flex: 1, flexWrap: 'wrap' }}>
          {links.map((l) => (
            <Link key={l.to} to={l.to} style={linkStyle(l.to)}
              onMouseOver={e => { if(!isActive(l.to)) { e.currentTarget.style.color = goldLight; e.currentTarget.style.background = 'rgba(201,168,76,0.07)'; }}}
              onMouseOut={e => { if(!isActive(l.to)) { e.currentTarget.style.color = '#bbb'; e.currentTarget.style.background = 'transparent'; }}}
            >
        ...
        {user && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexShrink: 0 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '6px 12px', borderRadius: 8, background: 'rgba(201,168,76,0.07)', border: '1px solid rgba(201,168,76,0.15)' }}>
              <div style={{ width: 28, height: 28, borderRadius: '50%', background: 'linear-gradient(135deg,#c9a84c,#f5d270)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <NavIcon type="user" />
              </div>
              <div>
                <div style={{ color: '#fff', fontSize: 12, fontWeight: 600, lineHeight: 1.2 }}>{user.name}</div>
